...
const buildBlockedSet = () => {
  ...
  blocked.add(getCellIndex(gameState.map.start.x, gameState.map.start.y));
  blocked.add(getCellIndex(gameState.map.exit.x, gameState.map.exit.y));
  return blocked;
};
...
const drawGrid = () => {
  ...
  for (let y = 0; y < grid.rows; y += 1) {
    for (let x = 0; x < grid.cols; x += 1) {
      ...
      ctx.strokeRect(px, py, grid.size, grid.size);
    }
  }
};
...
const drawMap = () => {
  ...
  ctx.fillRect(start.x * grid.size, start.y * grid.size, grid.size, grid.size);
  ctx.fillStyle = "#ffce45";
  ctx.fillRect(exit.x * grid.size, exit.y * grid.size, grid.size, grid.size);
};

const drawTowers = () => {
  gameState.towers.forEach((tower) => {
    const config = towerTypes[tower.type];
    const cx = tower.x * grid.size + grid.size / 2;
    const cy = tower.y * grid.size + grid.size / 2;

    ctx.fillStyle = "#0f131a";
    ctx.beginPath();
    ctx.arc(cx, cy, grid.size * 0.35, 0, Math.PI * 2);
    ctx.fill();

    ctx.strokeStyle = config.color;
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.arc(cx, cy, grid.size * 0.28, 0, Math.PI * 2);
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + Math.cos(tower.direction) * 14, cy + Math.sin(tower.direction) * 14);
    ctx.stroke();
  });
};
...
const drawEnemies = () => {
  gameState.enemies.forEach((enemy) => {
    ...
    ctx.beginPath();
    ctx.arc(px, py, grid.size * 0.22, 0, Math.PI * 2);
    ctx.fill();

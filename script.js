...
const selectedTowerEl = document.getElementById("selected-tower");
...
const setMessage = (text, tone = 520) => {
  messageEl.textContent = text;
  if (tone) playBeep(tone);
};
...
const playGunSound = (type) => {
  switch (type) {
    case "gatling":
      playSfx({ frequency: 680, duration: 0.07, volume: 0.05, slide: -180 });
      break;
    case "glue":
      playSfx({ frequency: 280, type: "triangle", duration: 0.12, volume: 0.06, slide: -40 });
      break;
    case "missile":
      playSfx({ frequency: 520, type: "sawtooth", duration: 0.15, volume: 0.08, slide: -220 });
      break;
    case "tesla":
      playSfx({ frequency: 900, type: "square", duration: 0.1, volume: 0.07, slide: -320 });
      break;
    default:
      playSfx({});
  }
...
const playExplosion = () => {
  playSfx({ frequency: 180, type: "sawtooth", duration: 0.2, volume: 0.09, slide: -60 });
};

const playLeak = () => {
  playSfx({ frequency: 140, type: "triangle", duration: 0.2, volume: 0.08, slide: -40 });
};

const playWaveStart = () => {
  playSfx({ frequency: 520, type: "square", duration: 0.12, volume: 0.07, slide: 160 });
};

const playGameOver = () => {
  playSfx({ frequency: 200, type: "square", duration: 0.3, volume: 0.08, slide: -80 });
};

const startMusic = () => {
  resumeAudio();
  if (musicTimer) return;
  const bassNotes = [110, 130.81, 146.83, 130.81];
  let step = 0;
...
const tryPlaceTower = (x, y) => {
  if (!canBuildAt(x, y)) {
    setMessage("Tile blocked. Try another spot.", 320);
    return;
  }
  const type = towerTypes[selectedTower];
  if (gameState.cash < type.cost) {
    setMessage("Not enough cash.", 260);
    return;
  }
  ...
  if (gameState.pathInvalid) {
    ...
    computeDistances();
    setMessage("You cannot block the only path.", 220);
    return;
  }

  gameState.cash -= type.cost;
  setMessage(`${type.name} deployed.`, 640);
};
...
const removeTower = (x, y) => {
  ...
  computeDistances();
  setMessage("Tower salvaged.", 300);
  playSfx({ frequency: 240, type: "triangle", duration: 0.1, volume: 0.05, slide: -30 });
};
...
const drawGridTile = (x, y) => {
  const px = x * grid.size;
  const py = y * grid.size;
  ctx.fillStyle = "#0b0f14";
  ctx.fillRect(px, py, grid.size, grid.size);
  ctx.fillStyle = "#0f151d";
  ctx.fillRect(px + 2, py + 2, grid.size - 4, grid.size - 4);
  ctx.strokeStyle = "#1b2431";
  ctx.strokeRect(px + 0.5, py + 0.5, grid.size - 1, grid.size - 1);
  ctx.strokeStyle = "rgba(255, 255, 255, 0.06)";
  ctx.strokeRect(px + 2.5, py + 2.5, grid.size - 5, grid.size - 5);
};
...
const drawMap = () => {
  ...
  walls.forEach(([x, y]) => {
    ...
    ctx.fillRect(px + 6, py + 6, grid.size - 12, grid.size - 12);
  });
  ...
  const drawPortal = (x, y, color) => {
    ...
    ctx.fillRect(px + 4, py + 4, grid.size - 8, grid.size - 8);
    ctx.fillStyle = color;
    ctx.fillRect(px + 8, py + 8, grid.size - 16, grid.size - 16);
  };

  drawPortal(start.x, start.y, "#3aa4ff");
  drawPortal(exit.x, exit.y, "#ffd166");
};

const drawPixelTower = (tower) => {
  const config = towerTypes[tower.type];
  ...
  ctx.lineWidth = 3;
  ctx.strokeRect(px + 7, py + 7, grid.size - 14, grid.size - 14);
  ...
  ctx.beginPath();
  ctx.moveTo(cx, cy);
  ctx.lineTo(cx + Math.cos(tower.direction) * 12, cy + Math.sin(tower.direction) * 12);
  ctx.stroke();
};

const drawTowers = () => {
  gameState.towers.forEach((tower) => {
    drawPixelTower(tower);
  });
};
...
const drawEnemies = () => {
  gameState.enemies.forEach((enemy) => {
    ...
    const size = grid.size * 0.4;
    ctx.fillStyle = enemy.kind === "air" ? "#46b0ff" : "#ff9f43";
...
const drawHover = (cell) => {
  ...
  ctx.strokeStyle = config.color;
  ctx.lineWidth = 2;
  ctx.strokeRect(cell.x * grid.size + 4, cell.y * grid.size + 4, grid.size - 8, grid.size - 8);
};
...
towerButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const config = towerTypes[selectedTower];
    selectedTowerEl.textContent = `${config.name}`;
    setMessage(`${config.name} selected.`, 420);
  });
});
...
const startGame = () => {
  ...
  selectedTowerEl.textContent = towerTypes[selectedTower].name;
  setMessage("Wave 1 ready. Build your maze.", 520);
};
